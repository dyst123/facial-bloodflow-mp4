...
class NewPhase3():
    ...
    def run(self, plot_fft=True):
        ...
        print("self.D shape:", np.shape(self.D), "type:", type(self.D))
        ...
        Green_raw_rec = np.squeeze(self.RGB[1])

        print("Depth_rec before smoothing:", Depth_rec, "Length:", len(Depth_rec))

        Depth_rec = np.atleast_1d(self.D)
        print("Depth_rec before smoothing:", Depth_rec, "Length:", len(Depth_rec))

        # Defensive: ensure not empty and at least window size
        if Depth_rec is None or len(Depth_rec) < 3:
            print("Warning: Depth_rec is empty or too short, skipping interval.")
            return 0
        
        plt.figure()
        plt.plot(Depth_rec, label='Depth Signal')
        plt.title(f'Depth Signal {self.clip}')
        plt.xlabel('Frame')
        plt.ylabel('Depth Value')
        plt.legend()
        plt.grid(True)
        plt.savefig(os.path.join(self.output_DIR, f'depth_signal_{self.clip}.png'), bbox_inches='tight')
        plt.close('all')
# Ensure window is odd and not longer than the signal
        window_length = min(9, len(Depth_rec) if len(Depth_rec) % 2 == 1 else len(Depth_rec) - 1)
        if window_length < 3:
            window_length = 3
        #Depth_rec = scipy.signal.savgol_filter(Depth_rec, window_length, 2, mode='nearest')
        I_raw_rec = self.safesavgol(I_raw_rec, window_length=11, polyorder=2)
        Green_raw_rec = self.safesavgol(Green_raw_rec, window_length=11, polyorder=2)
        ...
        # apply bandpass to remove <=45 BPM (keep >45 BPM)
        I_comp_rec = self.apply_bandpass_filter(I_comp_rec, self.fps, low_bpm=46, high_bpm=180, order=4)
        Green_comp_rec = self.apply_bandpass_filter(Green_comp_rec, self.fps, low_bpm=46, high_bpm=180, order=4)
        ...
        if self.Window:
            ...
            max_val_green = np.max(Green_comp_rec)
            I_comp_rec = (I_comp_rec - min_val) / (max_val - min_val)
            Green_comp_rec = (Green_comp_rec - min_val_green) / (max_val_green - min_val_green)

            window = np.hanning(len(I_comp_rec))
            window_green = np.hanning(len(Green_comp_rec))
            I_comp_rec = I_comp_rec * window
            Green_comp_rec = Green_comp_rec * window_green
        ...
        Green_freq2 = np.fft.fft(Green_raw_rec[:])
        X_final2 = np.abs(Intensity_freq2)
        X_final_green2 = np.abs(Green_freq2)
        freq2 = np.fft.fftfreq(len(X_final2), 1.0 / self.fps)*60.0
        freq2_green = np.fft.fftfreq(len(X_final_green2), 1.0 / self.fps)*60.0
        print("freq2: ", freq2)
        X_final2[np.where(freq2<40)]=0
        X_final2[np.where(freq2>190)]=0
        X_final_green2[np.where(freq2_green<40)]=0
        X_final_green2[np.where(freq2_green>190)]=0
        ...
        HR_green_2D = ((np.argmax(X_final_green2[math.floor(0.6667 * len(X_final_green2) / self.fps):math.ceil(2.5 * len(X_final_green2) / self.fps)]) + math.floor(
            0.6667 * len(X_final_green2) / self.fps))* self.fps * 60.0 / len(X_final_green2))
        
        hr_welch, freq_filtered_comp, hr_arr_comp = self.run_welch(I_raw_rec, self.fps)
        hr_welch_green, freq_filtered_comp_green, hr_arr_comp_green = self.run_welch(Green_raw_rec, self.fps)

        
        # process other info
        #RR, breathCount, blinks, perclos, bf, bf2 = self.processRest()
        RoR_RedvsIR, ROR_GreenvsIR = self.getRoR(self.RGB, self.I, self.fps)
        ...
        if plot_fft:
            ...
            plt.xlim(40, 150)
            HRs.append(HR)
            plt.title(f'FFT Spectrum of Compensated Intensity {self.clip}')
            plt.xlabel('Frequency (BPM)')
            plt.ylabel('Magnitude')
            plt.savefig(os.path.join(self.output_DIR, f'fft_spectrum_{self.clip}.png'), bbox_inches='tight')
            
            plt.figure()
            X_final_green[np.where(freq_green < 40)] = 0
            X_final_green[np.where(freq_green > 150)] = 0
            plt.plot(freq_green, X_final_green)
            plt.xlim(40, 150)
            HRs.append(HR_green)
            plt.title(f'FFT Spectrum of Compensated Green Intensity {self.clip}')
            plt.xlabel('Frequency (BPM)')
            plt.ylabel('Magnitude')
            plt.savefig(os.path.join(self.output_DIR, f'fft_spectrum_green_{self.clip}.png'), bbox_inches='tight')
            
            plt.figure()
            X_final2[np.where(freq2 < 40)] = 0
            X_final2[np.where(freq2 > 150)] = 0
            plt.plot(freq2, X_final2)
            plt.xlim(40, 150)
            HRs.append(HR_2D)
            plt.title(f'FFT Spectrum of Intensity 2D {self.clip}')
            plt.xlabel('Frequency (BPM)')
            plt.ylabel('Magnitude')
            plt.savefig(os.path.join(self.output_DIR, f'fft_spectrum_2D_{self.clip}.png'), bbox_inches='tight')
            
            plt.figure()
            X_final_green2[np.where(freq2_green < 40)] = 0
            X_final_green2[np.where(freq2_green > 150)] = 0
            plt.plot(freq2_green, X_final_green2)
            plt.xlim(40, 150)
            HRs.append(HR_green_2D)
            plt.title(f'FFT Spectrum of Green Intensity 2D {self.clip}')
            plt.xlabel('Frequency (BPM)')
            plt.ylabel('Magnitude')
            plt.savefig(os.path.join(self.output_DIR, f'fft_spectrum_green_2D_{self.clip}.png'), bbox_inches='tight')
            plt.close('all')

            plt.figure()
            plt.plot(Depth_rec, label='Depth Signal smoothed')
            plt.title(f'Depth Signal smoothed {self.clip}')
            plt.xlabel('Frame')
            plt.ylabel('Depth Value')
            plt.legend()
            plt.grid(True)
            plt.savefig(os.path.join(self.output_DIR, f'depth_signal_smoothed_{self.clip}.png'), bbox_inches='tight')
            plt.close('all')

            plt.figure()
            plt.plot(Green_comp_rec, label='Green Signal smoothed+rec')
            plt.title(f'Green Signal smoothed {self.clip}')
            plt.xlabel('Frame')
            plt.ylabel('Green Value')
            plt.legend()
            plt.grid(True)
            plt.savefig(os.path.join(self.output_DIR, f'green_signal_smoothed_{self.clip}.png'), bbox_inches='tight')
            plt.close('all')

            plt.figure()
            plt.plot(Green_raw_rec, label='Green Signal raw+rec')
            plt.title(f'Green Signal raw {self.clip}')
            plt.xlabel('Frame')
            plt.ylabel('Green Value')
            plt.legend()
            plt.grid(True)
            plt.savefig(os.path.join(self.output_DIR, f'green_signal_raw_rec_{self.clip}.png'), bbox_inches='tight')
            plt.close('all')
            
        return HR_2D, HR, Hr_stft, hr_welch, HR_green_2D, HR_green, Hr_stft_green, hr_welch_green
    
    #def processRest(self, plot_EAR=True):
        """
        ...
        if plot_EAR:
            ...
            plt.savefig(os.path.join(self.output_DIR, f'ear_values_{self.clip}.png'), bbox_inches='tight')
            plt.close('all')
    ...
    #def getRespitoryRate(self, chestDepths, Savgof=True, Lowpass=False, Window=False, realFFT=False, plot=True):
        """
        ...
        if plot:
            ...
            fig.show()
            plt.tight_layout()
            plt.savefig(os.path.join(self.output_DIR, f'chest_depth_fft_{self.clip}.png'), bbox_inches='tight')
            plt.close('all')
    ...
    def apply_lowpass_filter(self, sig, fps, low_bpm=46, high_bpm=200, order=4):
        """
        Zero-phase bandpass filter using second-order-sections. Removes frequencies <= low_bpm.
        low_bpm: lowest BPM to keep (so values <= low_bpm are attenuated)
        high_bpm: highest BPM to keep
        """
        ...
        if N < 4:
            return sig
        # convert bpm -> Hz
        low_hz = float(low_bpm) / 60.0
        high_hz = float(high_bpm) / 60.0
        nyq = 0.5 * fps
        ...
        if low_hz >= high_hz:
            return sig
        # design SOS bandpass with fs to use Hz units
        sos = signal.butter(order, [low_hz, high_hz], btype='bandpass', fs=fps, output='sos')
        # apply zero-phase filtering; fallback to forward/backward if sosfiltfilt fails
        try:
    ...
    def apply_bandpass_filter(self, sig, fps, low_bpm=46, high_bpm=180, order=4):
        """
        Zero-phase bandpass filter using SOS. low_bpm is the lowest BPM to keep (<= low_bpm will be attenuated).
        """
        sig = np.asarray(sig, dtype=np.float64)
        N = len(sig)
        if N < 4:
    ...
    def run_stft(self, D_signal_smooth, fps, I_avg, plot=True):
        """
        ...
        signal_len = len(I_avg[12:-12])
        if seg_length > signal_len:
            seg_length = signal_len
        if seg_overlap >= seg_length:
            seg_overlap = seg_length - 1 if seg_length > 1 else 0
        ...
        if plot:
            ...
            plt.legend()
            plt.tight_layout()
            plt.savefig(os.path.join(self.output_DIR, f'stft_{self.clip}.png'), bbox_inches='tight')
            plt.close('all')
    ...
    def get_I_avg(self, frame_num, fps, D_signal_smooth, I_signal_smooth):
        ...
        # use zero-phase filtering and use a cutoff that does not keep very low bpm if undesired
        I_avg = signal.sosfiltfilt(sos, I_avg)
    ...
    def getRoR(self, rgb, intensity, fps):
        """
        This function calculates the ratio of ratios (RoR) for the clip.
        The RoR is calculated by (AC_red / DC_red) / (AC_blue / DC_blue)
        
        Args:
            rgb (2D array of ints) : The RGB values of the clip
            fps (int) : The frames per second of the clip
            
        Returns:
            RoR (int) : The rate of redness
        """
        # Get DC Component
        DC_red = self.apply_lowpass_filter(rgb[0], 1, fps)
        DC_green = self.apply_lowpass_filter(rgb[1], 1, fps)
        DC_intensity = self.apply_lowpass_filter(intensity, 1, fps)
        
        # Get AC Component
        AC_red = np.abs(rgb[0] - DC_red)
        AC_green = np.abs(rgb[1] - DC_green)
        AC_intensity = np.abs(intensity - DC_intensity)
        
        # Calculate RoR
        DC_red_mag = np.mean(DC_red)
        DC_green_mag = np.mean(DC_green)
        AC_red_mag = np.percentile(AC_red, 90)
        AC_green_mag = np.percentile(AC_green, 90)
        DC_intensity_mag = np.mean(DC_intensity)
        AC_intensity_mag = np.percentile(AC_intensity, 90)
        ROR_RedvsIR = (AC_red_mag / DC_red_mag) / (AC_intensity_mag / DC_intensity_mag)
        ROR_GreenvsIR = (AC_green_mag / DC_green_mag) / (AC_intensity_mag / DC_intensity_mag)

        # Visualize all the components
        plt.figure()
        plt.plot(rgb[0], label='Red', color='r', linestyle='--')
        plt.plot(rgb[1], label='Green', color='g', linestyle='--')
        plt.plot(intensity, label='Intensity', color='black', linestyle='--')
        plt.plot(DC_red, label='DC Red')
        plt.plot(DC_green, label='DC Blue')
        plt.plot(DC_intensity, label='DC Intensity')
        plt.plot(AC_red, label='AC Red')
        plt.plot(AC_green, label='AC Blue')
        plt.plot(AC_intensity, label='AC Intensity')
        plt.legend()
        plt.title(f'RoR for {self.clip}')
        plt.xlabel('Frame')
        plt.ylabel('RGB Value')
        plt.savefig(os.path.join(self.output_DIR, f'RoR_{self.clip}.png'), bbox_inches='tight')
        plt.close('all')
        
        return ROR_RedvsIR, ROR_GreenvsIR
